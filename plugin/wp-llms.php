<?php
/**
 * Plugin Name:       WP LLMS - AI Discoverability for WordPress
    ...
    add_action('admin_notices', function (): void {
        printf(
            ...
            esc_html(sprintf(
                ...
                __('WP LLMS requires PHP %1$s or higher. You are running PHP %2$s.', 'wp-llms'),
                WPLLMS_MIN_PHP,
                PHP_VERSION
            ))
        );
    ...
    add_action('admin_notices', function () use ($wp_version): void {
        printf(
            ...
            esc_html(sprintf(
                ...
                __('WP LLMS requires WordPress %1$s or higher. You are running WordPress %2$s.', 'wp-llms'),
                WPLLMS_MIN_WP,
                $wp_version
            ))
        );
...
add_action('plugins_loaded', function (): void {
    load_plugin_textdomain('wp-llms', false, dirname(WPLLMS_BASENAME) . '/languages');
